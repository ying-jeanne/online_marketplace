...
DoorDash chose MAB with TS over other bandit algorithms because it naturally handles the exploration-exploitation trade-off through posterior sampling, which is computationally efficient for their large-scale system, and performs well empirically with binary outcomes (accept/reject). They preferred it over offline ML methods because TS can \textbf{learn and adapt in real-time} without a separate training phase, handles the cold-start problem for new Dashers through informative priors, and avoids the feedback loop issues of offline models that are trained on non-representative historical data. 
